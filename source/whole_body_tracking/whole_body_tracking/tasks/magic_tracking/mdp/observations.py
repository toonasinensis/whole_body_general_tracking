...
from whole_body_tracking.tasks.magic_tracking.mdp.commands import MotionCommand
...
def feet_contact_forces(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    返回双脚与地面接触时的力的大小（norm）。
    
    Args:
        env: 环境实例
        sensor_cfg: 传感器配置，需要指定双脚对应的body_ids
        
    Returns:
        形状为 (num_envs, num_feet) 的张量，包含每只脚的接触力大小
    """
    # 提取接触传感器
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    # 获取接触力历史数据
    net_contact_forces = contact_sensor.data.net_forces_w_history
    # 计算每个body的接触力的范数，并取历史中的最大值
