...
class MotionCommand(CommandTerm):
    ...
    def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRLEnv):
        ...
        # 动态计算 NPZ <-> Isaac Lab 关节映射（根据实际机器人 BFS 顺序，无需硬编码）
        lab_joint_names = list(self.robot.joint_names)
        npz_joint_names = list(PRESERVE_JOINT_ORDER_ASSET_CFG.joint_names)
        npz_to_isaac, isaac_to_npz = _build_joint_mapping(lab_joint_names, npz_joint_names)
        self._npz_to_isaac = torch.tensor(npz_to_isaac, dtype=torch.long, device=self.device)
        self._isaac_to_npz = torch.tensor(isaac_to_npz, dtype=torch.long, device=self.device)
        print(f"[MotionCommand] 关节映射已动态构建: lab={lab_joint_names}, npz={npz_joint_names}")
        print(f"[MotionCommand] npz_to_isaac={npz_to_isaac}")
        self.time_steps = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
        self.start_time = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)  # 轨迹开始前的计数器
        self.out_time = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)  # 轨迹结束后的计数器
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        if len(env_ids) == 0:
            return
        self._adaptive_sampling(env_ids)  # 采样起始帧（可能是0，也可能是中间帧）
        
        ############################测试
        # self.time_steps[env_ids] = 0
        ############################测试

        num_force_start = max(1, len(env_ids) // 16)  # 至少1个机器人从第0帧开始
        if num_force_start > 0:
            force_start_indices = torch.randperm(len(env_ids), device=self.device)[:num_force_start]
            force_start_env_ids = env_ids[force_start_indices]
            self.time_steps[force_start_env_ids] = 0
        # 根据采样的帧来决定 start_time 的初始值：
        # - 如果采样到第0帧，start_time = 0（将进入起始保持阶段）
        # - 如果采样到非第0帧，start_time = start_hold_steps（跳过起始保持阶段）
        ...
        # 获取IsaacLab顺序的关节限制，转换为NPZ顺序进行clip
        isaac_soft_joint_pos_limits = self.robot.data.soft_joint_pos_limits[env_ids]
        npz_soft_joint_pos_limits = isaac_soft_joint_pos_limits[:, self._isaac_to_npz, :]
        joint_pos[env_ids] = torch.clip(
            joint_pos[env_ids], npz_soft_joint_pos_limits[:, :, 0], npz_soft_joint_pos_limits[:, :, 1]
        )
        # 从NPZ顺序转换为IsaacLab顺序后写入仿真
        isaac_joint_pos = joint_pos[env_ids][:, self._npz_to_isaac]
        isaac_joint_vel = joint_vel[env_ids][:, self._npz_to_isaac]
    ...
    def _update_command(self):
        # 阶段1: 起始保持阶段（只在采样到第0帧时才会触发）
        # 条件：time_steps == 0 且 start_time < start_hold_steps
        # 行为：保持在第0帧，增加 start_time 计数器
        ...
        # 阶段2: 正常轨迹跟踪阶段
        # 条件：start_time >= start_hold_steps（已完成起始保持或跳过起始保持）
        # 行为：增加 time_steps，正常播放轨迹
        ...
        # 阶段3: 末尾保持阶段 (time_steps >= time_step_total - 1)
        # 保持在最后一帧，增加 out_time 计数器
        ...
        # 阶段4: 重新采样 (out_time > end_hold_steps)
