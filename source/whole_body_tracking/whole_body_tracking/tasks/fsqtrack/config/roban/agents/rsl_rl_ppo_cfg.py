...
# Command (s^g) dimension for S52 flat env: command_horizon * 2 * num_joints = 10 * 2 * 21 = 420.
# The runner builds the policy with eval("ActorCriticFSQVAE") from rsl_rl.modules.
ACTOR_SG_DIM_ROBAN = 420 # TODO the tracking dim remains to be modified
...
class RslRlFsqVaeActorCriticCfg:
    ...
    activation: str = "elu"
    actor_sg_dim: int = ACTOR_SG_DIM_ROBAN
...
@configclass
class FsqtrackRobanFlatPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 100000
    save_interval = 500
    experiment_name = "fsqtrackRoban_flat"
...
@configclass
class FsqtrackRobanFlatLowFreqPPORunnerCfg(FsqtrackRobanFlatPPORunnerCfg):
