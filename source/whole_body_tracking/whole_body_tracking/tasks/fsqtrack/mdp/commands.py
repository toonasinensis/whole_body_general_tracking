...
def reorder_joint_data_for_isaac(joint_data: torch.Tensor) -> torch.Tensor:
    ...
    if joint_data.shape[1] == 27:   # for kuavo s52
        NPZ_TO_ISAAC_INDICES = [0, 6, 12, 1, 7, 13, 20, 2, 8, 14, 21, 3, 9, 15, 22, 4, 10, 16, 23, 5, 11, 17, 24, 18, 25, 19, 26]
    elif joint_data.shape[1] == 21: # for roban s22
        NPZ_TO_ISAAC_INDICES = [0, 13, 17, 1, 7, 14, 18, 2, 8, 15, 19, 3, 9, 16, 20, 4, 10, 5, 11, 6, 12]
    else: # G1 of dof 29 remains to be modified
        raise ValueError(f"Unsupported joint data shape: {joint_data.shape[1]}")
...
def reorder_isaac_data_for_npz(isaac_data: torch.Tensor) -> torch.Tensor:
    ...
    if isaac_data.shape[1] == 27: # for kuavo s52
        ISAAC_TO_NPZ_INDICES = [0, 3, 7, 11, 15, 19, 1, 4, 8, 12, 16, 20, 2, 5, 9, 13, 17, 21, 23, 25, 6, 10, 14, 18, 22, 24, 26]
    elif isaac_data.shape[1] == 21: # for roban s22
        ISAAC_TO_NPZ_INDICES = [0, 3, 7, 11, 15, 17, 19, 4, 8, 12, 16, 18, 20, 1, 5, 9, 13, 2, 6, 10, 14]
    else: # G1 of dof 29 remains to be modified
        raise ValueError(f"Unsupported isaac data shape: {isaac_data.shape[1]}")
