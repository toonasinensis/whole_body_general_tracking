...
"""Script to train SON (multi-motion) motion-tracking policy with RSL-RL."""
...
parser = argparse.ArgumentParser(description="Train a SON RL agent with RSL-RL.")
...
# support local motion dataset and optional registry, similar to Sonic script
parser.add_argument(
    "--motion_path",
    type=str,
    default=None,
    help=(
        "Path to local motion data: "
        "- for single-motion commands: a .npz file; "
        "- for MultiMotionCommand: a dataset directory (overrides registry)."
    ),
)
parser.add_argument(
    "--registry_name",
    type=str,
    default=None,
    help="The name of the wandb registry (if not using --motion_file).",
)
...
# Import extensions to set up environment tasks (includes SON env registration)
import whole_body_tracking.tasks  # noqa: F401
# Explicitly import SON Kuavo S52 config to ensure Gym env registration
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    """Train SON motion-tracking policy with RSL-RL agent."""
    ...
    # load motion data
    # Prefer local path if provided; otherwise, fall back to WandB registry
    if args_cli.motion_path is not None:
        ...
        motion_cfg = env_cfg.commands.motion
        if hasattr(motion_cfg, "motion_file"):
            # Conventional single-motion artifact layout: motion.npz inside root
            motion_cfg.motion_file = str(artifact_root / "motion.npz")
        elif hasattr(motion_cfg, "dataset_dir"):
            # MultiMotionCommand (SON): treat artifact root as dataset directory
