...
class MotionLoader:
    def __init__(
        ...
        for i in range(self.motion_num):
            ...
            st = time.time()
            motion_base_poss, motion_base_rots, motion_dof_poss = self._interpolate_motion(self.duration_list[i], self.output_dt, self.body_pos_w_list[i][0],\
                                     self.body_quat_w_list[i][0], self.joint_pos_list[i][0])
            motion_base_lin_vels, motion_base_ang_vels, motion_dof_vels = self._compute_velocities(motion_base_poss, motion_base_rots, motion_dof_poss, self.output_dt)
            ...
            et = time.time()
            print(f"motion {i} processed, duration: {self.duration_list[i]:.2f} sec, input frames: {self.input_frames_list[i]}, output frames: {frame_out_num}, time cost: {et-st:.2f} sec")
    ...
    def _load_motion(self):
        ...
        if ext == ".csv":
            ...
            for data in data_list:
                ...
                fps = torch.tensor(data["fps"], dtype=torch.float32, device=self.device)  # (1,)
            ...
            # padded_q[:num_frame] = b_quat
                ...
                self.duration_list.append((num_frame - 1) / fps)
                self.input_frames_list.append(num_frame)
                self.motion_num = len(data_list)
            print(f"Motion loaded ({self.motion_file}), duration: {self.duration_list} sec, frames: {self.input_frames_list}")
            ...
            for f_path in npz_file_paths:
                ...
                self.data_joint_order =  ['left_hip_pitch_joint', 'left_hip_roll_joint', 'left_hip_yaw_joint', 'left_knee_joint', \
                'left_ankle_pitch_joint', 'left_ankle_roll_joint', 'right_hip_pitch_joint', 'right_hip_roll_joint', 'right_hip_yaw_joint', \
                'right_knee_joint', 'right_ankle_pitch_joint', 'right_ankle_roll_joint', 'waist_yaw_joint', 'waist_roll_joint', \
                'waist_pitch_joint', 'left_shoulder_pitch_joint', 'left_shoulder_roll_joint', 'left_shoulder_yaw_joint', \
                'left_elbow_joint', 'left_wrist_roll_joint', 'left_wrist_pitch_joint', 'left_wrist_yaw_joint',\
                 'right_shoulder_pitch_joint', 'right_shoulder_roll_joint', 'right_shoulder_yaw_joint', \
                'right_elbow_joint', 'right_wrist_roll_joint', 'right_wrist_pitch_joint', 'right_wrist_yaw_joint']
                data_list.append(data)
            # import ipdb;ipdb.set_trace()    
            max_frame = max(data["qpos"].shape[0] for data in data_list)
    ...
    def _interpolate_motion(self, duration, output_dt, motion_base_poss_input, motion_base_rots_input, motion_dof_poss_input):
    ...
    def get_next_state(
        ...
        print("仿真进度： ", float(self.current_idx[max_idx] / self.lerpfn[max_idx]) , "max lengh :", self.lerpfn[max_idx])
...
def run_simulator(motion:MotionLoader, sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
...
def main():
    ...
    motion = MotionLoader(
        ...
        motion_file="/home/thl/wt_wbc/wbc_parkour/whole_body_tracking/data/terrain",
