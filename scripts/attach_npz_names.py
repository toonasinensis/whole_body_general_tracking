...
def _names_for_args(args: argparse.Namespace, joint_dim: int, body_dim: int) -> tuple[list[str], list[str]]:
    joint_names = _read_names_file(args.joint_names_file)
    ...
    if body_names is None:
        ...
        if body_order == "auto":
            if body_dim == len(G1_MOTION_BODY_NAMES):
                body_order = "g1_motion"
            elif body_dim == len(G1_FULL_ISAAC_BODY_NAMES):
                body_order = "g1_full_isaac"
            else:
                raise ValueError(
                    f"Cannot auto-select body order for body_pos_w dim {body_dim}. "
                    "Pass --body_names_file with the exact npz body order."
                )
        if body_order == "g1_motion":
            body_names = list(G1_MOTION_BODY_NAMES)
        elif body_order == "g1_full_isaac":
            body_names = list(G1_FULL_ISAAC_BODY_NAMES)
        elif body_order == "g1_full_mjcf":
            body_names = list(G1_FULL_MJCF_BODY_NAMES)
        else:
            raise ValueError(f"Unsupported --body_order {args.body_order!r}.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Attach joint_names/body_names metadata to existing motion npz files.")
    ...
    parser.add_argument(
        "--body_order",
        default="auto",
        choices=["auto", "g1_motion", "g1_full_isaac", "g1_full_mjcf"],
    )
