...
def _unwrap_policy_obs(obs):
    """Extract the policy observation tensor from various env return types."""
    # common pattern in IsaacLab: (obs, extras) or (obs, info)
    if isinstance(obs, tuple) and len(obs) > 0:
        obs = obs[0]
    # some envs return a dict of observation groups
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    obs = _unwrap_policy_obs(env.reset())
    timestep = 0
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # rsl-rl vec env: (obs, reward, done, info)
            obs = step_out[0]
        if args_cli.video:
            timestep += 1
