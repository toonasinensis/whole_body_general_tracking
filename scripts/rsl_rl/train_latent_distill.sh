...
RESUME_TEACHER_PATH="${RESUME_TEACHER_PATH:-/home/thl/wt_wbc/wbc_parkour/whole_body_tracking/logs/rsl_rl/model_46500.pt}"
...
if [[ -z "${RESUME_TEACHER_PATH}" ]]; then
  echo "RESUME_TEACHER_PATH must point to a teacher checkpoint." >&2
  exit 1
fi
...
"${PYTHON_CMD[@]}" -m torch.distributed.run \
  ...
  "${PROJECT_DIR}/scripts/rsl_rl/train_distill.py" \
  --registry_name="${REGISTRY_NAME}" \
  --task="${TASK}" \
  --headless \
  --distributed \
  --num_envs="${NUM_ENVS}" \
  --resume_teacher_path="${RESUME_TEACHER_PATH}" \
  "${EXTRA_ARGS[@]}"
