...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Disable external disturbances (push, force/torque) during play.",
)
# Optional override flag for clarity and future parity with FSQ play script.
parser.add_argument(
    "--disturbance",
    action="store_true",
    default=False,
    help="Enable external disturbances during play (default behaviour).",
)
parser.add_argument(
    "--no_timeout",
    action="store_true",
    default=False,
    help="Disable max-episode-length time-out termination during play.",
)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # Optionally disable external disturbances (push, external force/torque) for clean play.
    if args_cli.no_disturbance and not args_cli.disturbance:
        _disable_disturbances(env_cfg)

    # Optionally disable time-out termination (max episode length).
    if args_cli.no_timeout:
        _disable_time_out(env_cfg)
...
def _disable_time_out(env_cfg):
    """Remove time-out termination so episodes are not cut by max length."""
    terminations = getattr(env_cfg, "terminations", None)
    if terminations is None:
        return
    if hasattr(terminations, "time_out"):
        delattr(terminations, "time_out")
