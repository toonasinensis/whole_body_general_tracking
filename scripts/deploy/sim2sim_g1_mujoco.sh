...
TASK="${TASK:-AMP-G1}"
NUM_ENVS="${NUM_ENVS:-1}"
RESUME_PATH="${RESUME_PATH:-${ROOT_DIR}/logs/rsl_rl/g1_amp/2026-06-23_10-50-47/model_62500.pt}"
MOTION_FILE="${MOTION_FILE:-/home/kiki/workspace/motion_data/mimic_data/g1/lafan}"
DATASET_TXT="${DATASET_TXT:-dataset_txt/fall.txt}"
...
${SIM2SIM_PYTHON} scripts/deploy/sim2sim_g1_mujoco.py \
  --onnx_path "${ONNX_PATH}" \
  --motion_file "${MOTION_FILE}" \
  "${DATASET_ARGS[@]}" \
  "${MOTION_SELECT_ARGS[@]}" \
