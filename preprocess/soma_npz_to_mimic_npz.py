...
class MotionLoader:
    ...
    def _load_motion(self):
        ...
        expected_dof_dim = len(PRESERVE_JOINT_ORDER_ASSET_CFG.joint_names or [])
...
def run_simulator_batch(
    ...
    while simulation_app.is_running():
        ...
        for env_id, motion in enumerate(motions):
            ...
            # append logs for this env
            rjp = reorder_joint_data(robot.data.joint_pos[env_id, :], robot, joint_names)
            rjv = reorder_joint_data(robot.data.joint_vel[env_id, :], robot, joint_names)
            logs[env_id]["joint_pos"].append(rjp.cpu().numpy().copy())
            logs[env_id]["joint_vel"].append(rjv.cpu().numpy().copy())
...
def run_simulator_single(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
    ...
    while simulation_app.is_running():
        ...
        rjp = reorder_joint_data(robot.data.joint_pos[0, :], robot, joint_names)
        rjv = reorder_joint_data(robot.data.joint_vel[0, :], robot, joint_names)
        log["joint_pos"].append(rjp.cpu().numpy().copy())
        log["joint_vel"].append(rjv.cpu().numpy().copy())
