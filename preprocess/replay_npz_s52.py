...
parser.add_argument("--npzfile", type=str, required=True, help="npzfile absolute/path/xxx.npz")
...
# Pre-defined configs
##
import whole_body_tracking.robots.kuavo_s52 as kuavo_assets
from whole_body_tracking.robots.kuavo_s52 import KUAVO_S52_CFG
...
def reorder_joint_data_for_isaac(joint_data: torch.Tensor, npz_joint_order: list[str],
                                 isaac_joint_names: list[str]) -> torch.Tensor:
    """
...
class ReplayMotionsSceneCfg(InteractiveSceneCfg):
    ...
    # articulation
    robot: ArticulationCfg = KUAVO_S52_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    motion_file = str(pathlib.Path(npzfile))
    npz_joint_order = kuavo_assets.KUAVO_S52_PRESERVE_JOINT_ORDER_ASSET_CFG.joint_names
    ...
    while simulation_app.is_running():
        ...
        sim.set_camera_view(pos_lookat + np.array([-4.0, -4.0, 0.5]), pos_lookat)
