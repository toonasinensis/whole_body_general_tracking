...
class Discriminator(nn.Module):
    ...
    def compute_loss(self, policy_d, expert_d, sample_amp_expert, _sample_amp_policy=None, lambda_=10):
        ...
        Args:
            policy_d: 策略轨迹的判别器输出。
            expert_d: 专家轨迹的判别器输出。
            sample_amp_expert: (expert_state, expert_next_state) 元组。
            sample_amp_policy: (policy_state, policy_next_state) 元组，保留接口备用。
            lambda_: 梯度惩罚系数。

        Returns:
            (amp_loss, grad_pen_loss): 均为标量张量。
        """
        ...
        grad_pen = self.compute_grad_pen(*sample_amp_expert, lambda_=lambda_)
